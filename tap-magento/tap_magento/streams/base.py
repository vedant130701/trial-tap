        ...
        if value_lower and value_upper:
            if value_lower == value_upper:
                ...
                raise ValueError((
                    ...
                    "values. Contact Magento/PayPal support.").format(
                        transaction_dict['id'],
                        field_name_upper,
                        field_name))
        elif value_upper:
...
def magento_error_handling(fnc):
    @backoff.on_exception(backoff.expo,
                          (http.client.IncompleteRead, ConnectionResetError),
                          max_tries=MAX_RETRIES,
                          factor=2)
    @backoff.on_exception(backoff.expo, # timeout error raise by Magento
...
class Stream():
    ...
    # with magento_error_handling to get 429 and 500 handling.
    @magento_error_handling
